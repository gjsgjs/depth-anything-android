...
class DepthAnything(context: Context, val modelName: String) {
    ...
    // 添加水平翻转
    private val flipTransform = Matrix().apply { postScale(-1f, 1f, outputDim / 2f, outputDim / 2f) }
    suspend fun predict(inputImage: Bitmap): Pair<Bitmap, Long> =
        withContext(Dispatchers.Default) {
            ...
            depthMap = Bitmap.createBitmap(depthMap, 0, 0, outputDim, outputDim, flipTransform, false)
